    ...
    Future<List<KonsumsiKalori>> getAllKonsumsiKalori() async {
        ...
        if (response.statusCode == 200) {
            ...
            final List<dynamic> rawData = responseData['data']['konsumsi_kalori'] ?? [];
            return rawData.map((json) => KonsumsiKalori.fromJson(json)).toList();
        } else {
            throw Exception(jsonDecode(response.body)['message'] ?? 'Gagal mengambil data konsumsi kalori');
        }
        } catch (e) {
    ...
    Future<List<KonsumsiKalori>> getKonsumsiKaloriToday() async {
        ...
        final headers = await _getHeaders();
        final response = await http.get(
            Uri.http(AppConfig.API_HOST, '/api/KonsumsiKalori/konsumsi-kaloriToday'),
            headers: headers,
        );
        ...
        if (response.statusCode == 200) {
            ...
            throw Exception(jsonDecode(response.body)['message'] ?? 'Gagal mengambil data konsumsi kalori hari ini');
        }
        } catch (e) {
    ...
    Future<KonsumsiKalori> createKonsumsiKalori(KonsumsiKalori konsumsi) async {
        ...
        if (response.statusCode == 200) {
            ...
            throw Exception(jsonDecode(response.body)['message'] ?? 'Gagal menambahkan konsumsi kalori');
        }
        } catch (e) {
    ...
    Future<KonsumsiKalori> getKonsumsiKaloriById(String id) async {
        ...
        if (response.statusCode == 200) {
            ...
            throw Exception(jsonDecode(response.body)['message'] ?? 'Konsumsi kalori tidak ditemukan');
        }
        } catch (e) {
        throw Exception('Error fetching konsumsi kalori: $e');
        }
    ...
    Future<KonsumsiKalori> updateKonsumsiKalori(String id, KonsumsiKalori konsumsi) async {
        try {
        final headers = await _getHeaders();
        ...
        if (response.statusCode == 200) {
            ...
            throw Exception(jsonDecode(response.body)['message'] ?? 'Gagal memperbarui konsumsi kalori');
        }
        } catch (e) {
    ...
    Future<void> deleteKonsumsiKalori(String id) async {
        ...
        if (response.statusCode != 200) {
            throw Exception(jsonDecode(response.body)['message'] ?? 'Gagal menghapus konsumsi kalori');
        }
        } catch (e) {
    ...
    Future<List<Map<String, dynamic>>> getWeeklySummary() async {
        ...
        final headers = await _getHeaders();
        final response = await http.post(
            Uri.http(AppConfig.API_HOST, '/api/KonsumsiKalori/konsumsi-kalori/weekly'),
            headers: headers,
        );
        ...
        if (response.statusCode == 200) {
            ...
            throw Exception(jsonDecode(response.body)['message'] ?? 'Gagal mengambil ringkasan mingguan');
        }
        } catch (e) {
    ...
    // Update summary
    Future<void> updateSummary(String date) async {
        try {
        final headers = await _getHeaders();
        final response = await http.post(
            Uri.http(AppConfig.API_HOST, '/api/KonsumsiKalori/konsumsi-kalori/update'),
            headers: headers,
            body: jsonEncode({'tanggal': date}),
        );

        if (response.statusCode != 200) {
            throw Exception(jsonDecode(response.body)['message'] ?? 'Gagal memperbarui ringkasan konsumsi');
        }
        } catch (e) {
    ...
    Future<Map<String, dynamic>> getKonsumsiByDayAndCategory(String date, String category) async {
        try {
        final headers = await _getHeaders();
        final response = await http.post(
            Uri.http(AppConfig.API_HOST, '/api/KonsumsiKalori/konsumsi-kalori/by-day-category'),
            headers: headers,
            body: jsonEncode({
            'date': date,
            'category': category,
